...
## 开发文档：想做一个自己的 ZCode 插件？

本插件的开发过程沉淀了一套完整的开发文档，按你的情况选：

| 文档 | 适合谁 |
|------|--------|
| [01 - 从零发布你的第一个 ZCode 插件](docs/01-从零发布你的第一个ZCode插件.md) | 第一次上手：30~60 分钟跟做出第一个插件，走完写 → 装 → 测 → 发布 → 更新全链路 |
| [02 - ZCode 插件开发实战指南](docs/02-ZCode插件开发实战指南.md) | 已经在写插件：安装/更新/Skill 触发/排错的按症状速查手册 |
| [03 - Agent 插件开发通用方法论](docs/03-Agent插件开发通用方法论.md) | 想少踩坑：真实生效点、架构模式、两层验证、调试四步法等跨平台通用原则 |

[→ 查看开发文档导航](docs/README.md)
